...
from auth.auth import get_current_active_user, require_paid_user
...
async def generate_trend_analysis(
    ...
    # Check if user has access to AI analysis (Pro subscription)
    # if not user_service.user_has_pro_access(cast(int, current_user.id)):
    #     raise HTTPException(
...
async def generate_peer_group_analysis(
    ...
    # Check if user has access to peer group analysis (Pro subscription)
    # if not user_service.user_has_pro_access(cast(int, current_user.id)):
    #     raise HTTPException(
    #         status_code=status.HTTP_403_FORBIDDEN,
...
async def calculate_dcf(
    ...
    """Calculate DCF valuation for a company (Pro)."""
    # Check API usage limits
    user_service = UserService(db)
    usage = user_service.check_api_usage_limit(cast(int, current_user.id))
...
async def reverse_dcf(
    ...
    market price (Pro). Other three assumptions are held constant.
...
async def calculate_wacc(
    ...
    """Compute WACC for a single filing (Pro).
...
async def generate_relative_valuation(
    ...
    """Three-layer relative valuation (Pro):
...
async def screener(
    ...
    """Paginated industry-wide screener (Pro). Reads from the precomputed cache —
...
async def screener_options(
    ...
    """Distinct values available for the screener filters (Pro; populated from the
