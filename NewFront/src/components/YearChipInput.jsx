...
const YearChipInput = forwardRef(({
  value = [],
  onChange,
  reportType = '10-K',
  placeholder,
  ...
  // Report-type switches change the parser; drop any in-progress text so a
  // stale partial (e.g. "2024" while on 10-Q) doesn't block the field.
  useEffect(() => {
    setInput('');
    setTouched(false);
  }, [reportType]);

  const commitPending = (focus = false) => {
    ...
    const next = value.includes(norm) ? value : [...value, norm];
  ...
  useImperativeHandle(ref, () => ({
    commitPending: () => commitPending(false),
  }));
...
YearChipInput.displayName = 'YearChipInput';
